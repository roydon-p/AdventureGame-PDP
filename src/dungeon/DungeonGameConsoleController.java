    ...
    try {
      ...
      while (true) {
        ...
        out.append("You are in a " + g.getLocationType(g.getPlayerLocation())).append("\n")
                .append("Doors lead to: " + g.getNextPossibleMoves()).append("\n")
                .append("You have: " + g.getPlayerDescription()).append("\n");

        String items = g.getAvailableItems();
        if (g.getRubyCount() > 0 || g.getDiamondCount() > 0 || g.getSapphireCount() > 0
                || g.getArrowCount() > 0) {
          out.append("You find " + items + "here").append("\n");
        }

        String smell = g.getLocationSmell();
        if (!smell.equals("")) {
              ...
              try {
                ...
                out.append("Not a valid direction or distance. ")
                        .append("Please ensure that the distance is between 1 and 5 and direction"
                                + " is valid: " + g.getNextPossibleMoves()).append("\n");
              }
            }
            break;
          default:
